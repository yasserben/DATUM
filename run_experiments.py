...
if __name__ == '__main__':
    ...
    if args.exp is not None:
        ...
        for i, cfg in enumerate(cfgs):
            if args.debug:
                ...
                cfg['evaluation'] = dict(interval=50, metric='mIoU', save_best='mIoU')
                if 'dacs' in cfg['name']:
                    cfg.setdefault('uda', {})['debug_img_interval'] = 10
                    # cfg.setdefault('uda', {})['print_grad_magnitude'] = True
            # Generate Config File
            cfg['name'] = f'{datetime.now().strftime("%m%d_%H%M")}_' \
                          f'{cfg["name"]}_{str(uuid.uuid4())[:5]}'
    ...
    if args.machine == 'local':
        for i, cfg in enumerate(cfgs):
            ...
            train.main([config_files[i]], data_root=args.data_root)
