...
def generate_experiment_cfgs(id):
    ...
    def config_from_vars():
        ...
        cfg['evaluation'] = dict(interval=iters // 20, metric='mIoU', save_best='mIoU')
    ...
    # DAFormer+DATUM on GTA→Cityscapes with the MiT-B5 encoder
    if id == 1 :
        iters = 40000
        seed = 0
        (source, target) = ('gta', 'db_data')
    ...
    # DAFormer+DATUM on GTA→Cityscapes with the ResNet-101 encoder
    elif id == 2 :
        iters = 40000
        seed = 0
        (source, target) = ('gta', 'db_data')
        ...
        (architecture, backbone) = ('dlv2red', 'r101v1c')
    ...
    # DAFormer+DATUM on SYNTHIA→Cityscapes with MiT-B5 encoder
    elif id == 3 :
        iters = 40000
        seed = 0
        (source, target) = ('synthia', 'db_data')
        (architecture, backbone) = ('daformer_sepaspp', 'mitb5')
        uda = 'dacs_a999_fdthings'
        rcs_T = 0.01
        plcrop = True
    ...
    # DAFormer+DATUM on SYNTHIA→Cityscapes with ResNet-101 encoder
    elif id == 4 :
        iters = 40000
        seed = 0
        (source, target) = ('synthia', 'db_data')
